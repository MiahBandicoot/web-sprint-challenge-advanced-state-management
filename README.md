...
Keeps State 'Clean' when data needs to be passed through intermediate components.

2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

Actions pass data through a reducer to update state according to 'type'. Reducers are functions that don't cause side effects and only return a new object. The Store is used to hold state globally.

3. What is the difference between Application state and Component state? When would be a good time to use one over the other?

Application State is stored on a server, best used to handle data such as user sign in data. Component state is created with setState, best for dealing with multiple changes to state such as a updating database.

4. Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

Redux-Thunk makes a synchronous reducer follow an asynchronous flow.

5. What is your favorite state management system you've learned and this sprint? Please explain why!

Redux, mainly because I have better understanding of it at the moment.
